...
If as a wallet operator, you are presented an invoice that includes `route_hints` AND a `destination` pubkey that does not appear in the graph, we recommend using a special type of prepay probe that checks the route to the final hop node before the destination wallet, and then computes what the total fee will be paid for the user using the provided information.&#x20;
...
> amt=payment_request.num_satoshis \
...
```
final_hop_fee = payment_request.route_hints.hop_hints.fee_base_msat \
> + payment_request.num_satoshis * payment_request.route_hints.hop_hints.fee_proportional_millionths / 1000000
```
